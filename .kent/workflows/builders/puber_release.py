...
CI_AGENT_ENTRY_PROMPTS["ci_watch_state_changed"] = 'Perform read-only PR/CI reconciliation in waiting_pr. Treat state_changed as an observation, not successful CI or release acceptance. Reconcile the observed PR head/base and CI state before choosing the existing waiting_pr transition. For source_changed, route through waiting_pr_ci_recheck to ci_prepare to reprepare and recheck CI for the changed source. Request waiting_pr_ci_recheck whenever the observed state requires renewed CI. Carried ci_contract and ci_report are historical opaque inputs, never successful CI evidence for the newly observed head/base. Preserve the complete flat release/CI identity, explicit verification summary and cursor. Preserve merge/tag/publication approval gates; do not merge, tag or publish. Only merge_watch_pr_merged may request Publish approval.\n\nworkspace_path: {{.Params.workspace_path}}\noperation_id: {{.Params.operation_id}}\ntask_short_id: {{.Params.task_short_id}}\nrelease_type: {{.Params.release_type}}\nrelease_version: {{.Params.release_version}}\nrelease_tag: {{.Params.release_tag}}\nrelease_branch: {{.Params.release_branch}}\nrelease_base_oid: {{.Params.release_base_oid}}\ncandidate_oid: {{.Params.candidate_oid}}\nrelease_head_oid: {{.Params.release_head_oid}}\nprofile_report: {{.Params.profile_report}}\nprofile_report_digest: {{.Params.profile_report_digest}}\nrelease_preparation_report: {{.Params.release_preparation_report}}\nrelease_preparation_report_digest: {{.Params.release_preparation_report_digest}}\nverification_summary: {{.Params.verification_summary}}\npr_url: {{.Params.pr_url}}\nbranch_name: {{.Params.branch_name}}\nmerge_strategy: {{.Params.merge_strategy}}\npr_head_oid: {{.Params.pr_head_oid}}\npr_base_oid: {{.Params.pr_base_oid}}\npr_report: {{.Params.pr_report}}\nci_contract: {{.Params.ci_contract}}\nci_report: {{.Params.ci_report}}\npr_feedback_cursor: {{.Params.pr_feedback_cursor}}'
...
def check_graph(path: Path) -> list[str]:
    ...
    for edge in edges:
        ...
        if target.get("kind") == "agent" and (
            not isinstance(prompt, str) or not prompt.strip()
        ):
            errors.append(f"Agent entry requires a nonempty prompt: {edge.get('key')}")
